...
public class RadialStatementNode extends DCNode {
	...
	public RadialStatementNode(String gId, String rId, String nId, DCNode r, DCNode n,  DCNode bl, Scope s, int l, int c) {
		groupId = gId;
		radId= rId;
		numId= nId;
		radExpr=r;
	...
	public VarType evaluate() {
		...
		for(double i = start; i < stop; i+=increment) {
			...
			if(d.size()>0){
				...
				g.heading(theta, rad);
				g.rotateWithFocus(theta,g.getOrigin(),true);
				group.addToGroup(g);
